 ...
 *
 * https://gitlab.com/eyeo/adblockplus/abp-snippets/-/tree/next/test/runners/download.js
 * except for:
...
export async function download(url, destFile) {
  ...
  const tempDest = `${destFile}-${process.pid}`;
  const writable = fs.createWriteStream(tempDest);
