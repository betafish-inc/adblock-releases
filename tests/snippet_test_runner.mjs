...
import chromiumRemoteProcess from "./snippets/runners/chromium_remote_process.mjs";
import chromiumProcess from "./snippets/runners/chromium_process.mjs";
import firefoxProcess from "./snippets/runners/firefox_process.mjs";
...
function configureRunners() {
  ...
  if (runners.length == 0) {
    ...
    // as it fails.
    if (os.type() === "Linux" && os.release().includes('Microsoft')) {
      return ["chromium"];
    }
...
async function runBrowserTests(processes) {
  ...
  let bundle = await webpackInMemory(bundleFilename, {
    entry: path.join(dirname, "snippets", "browser", "_bootstrap.js"),
    module: {
      rules: [
        {
  ...
  let localhost = await spawn(
    "node",
    [path.join(dirname, "snippets", "browser", "_server.mjs")]
  );
  ...
  await Promise.all(
    processes.map(
      currentProcess => runnerDefinitions[currentProcess](
        ...
        browserFiles.map(
          file => path.relative(path.join(dirname, "snippets", "browser"), file)
            .replace(/\.js$/, "")
        )
      )
...
(async function () {
  ...
  let paths = args.length > 2 ? args :
    [path.join(dirname, "snippets", "browser")];
