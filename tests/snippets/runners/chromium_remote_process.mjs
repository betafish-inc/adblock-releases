 ...
 *
 * https://gitlab.com/eyeo/adblockplus/abp-snippets/-/tree/next/test/runners/chromium_remote_process.js
 * except for:
 * - lint / style modifications
 * - added support of '--debugPrint' option
 * 
*/
...
function startChromium(chromiumPath) {
  ...
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'chromium-data'));
  let child = null;
  return {
...
function throwException(details, url) {
  ...
  if (!details.stackTrace) {
    ...
    text += `\n    at ${details.url || url}:${details.lineNumber + 1}:${details.columnNumber + 1}`;
  }
  throw text;
}
...
function connectRemoteInterface(attemptParam) {
...
function runScript(script, scriptName, scriptArgs) {
  return connectRemoteInterface().then(async (client) => {
    try {
      ...
      const index = args.indexOf('--debugPrint');
      if (index !== -1) {
      ...
      const compileResult = await Runtime.compileScript({
        expression: script,
      ...
      const runResult = await Runtime.runScript({
      ...
      const callResult = await Runtime.callFunctionOn({
      ...
      const result = await Runtime.evaluate({
        expression: 'window.testingConsoleLogs',
        returnByValue: true,
      });

      console.log('\n Browser tests in Chromium (Remote Interface)');

      for (const item of result.result.value.log) {
        console.log(...item);
      }
...
export default function (script, scriptName, ...scriptArgs) {
  return ensureChromium(CHROMIUM_REVISION).then((chromiumPath) => {
    const child = startChromium(chromiumPath);
    return Promise.race([
      child.done,
