...
const gabHostnamesWithProtocal = [
  'https://getadblock.com',
  'http://dev.getadblock.com',
  'http://dev1.getadblock.com',
  'http://dev2.getadblock.com',
...
function onMessage(request, sender, sendResponse) {
  if (Object.prototype.hasOwnProperty.call(request, 'dataMigrationStatus')) {
    ...
    window.postMessage(
      { dataMigrationStatus: request.dataMigrationStatus },
      '*'
    )
    sendResponse({})
  }
...
;(function onLoad() {
  ...
  if (
    gabHostnames.includes(document.location.hostname) &&
...
const runBandaids = function () {
  ...
  const bandaids = {
    ...
    getadblock() {
      ...
      if (enableShowSurvey) {
        enableShowSurvey.onclick = function showSurvey() {
          browser.runtime.sendMessage({
            command: 'setSetting',
            name: 'show_survey',
            isEnabled: !enableShowSurvey.checked,
          })
        }
      }
      ...
      if (aaElements && aaElements.length) {
        for (let i = 0; i < aaElements.length; ++i) {
          aaElements[i].onclick = function unsubscribeAcceptableAds(event) {
            ...
            browser.runtime
              .sendMessage({ command: 'unsubscribe', id: 'acceptable_ads' })
              .then(() => {
                browser.runtime
                  .sendMessage({
                  ...
                  .then(() => {
                    browser.runtime.sendMessage({
                      command: 'openTab',
                      urlToOpen: 'options.html?aadisabled=true#general',
                    })
                  })
              })
          }
      ...
      const getStartedElements = document.querySelectorAll(
        '.get-started-with-myadblock'
      )
      if (getStartedElements && getStartedElements.length) {
        for (let i = 0; i < getStartedElements.length; ++i) {
          getStartedElements[i].onclick = function getStartedWithMyAdBlock(
            event
          ) {
            if (event.isTrusted === false) {
              return
      ...
      document.addEventListener(
        ...
        (event) => {
          ...
          if (link.protocol === 'http:' || link.protocol === 'https:') {
            if (
              link.host === 'subscribe.getadblock.com' &&
              link.pathname === '/'
            ) {
