...
import {
  ...
  URLFilter,
  BlockingFilter,
} from '../../adblockplusui/adblockpluschrome/adblockpluscore/lib/filterClasses.js'
...
import {
  checkAllowlisted,
  getKey,
} from '../../adblockplusui/adblockpluschrome/lib/allowlisting.js'
...
let resourceTypes = new Map(
  (function* () {
    for (let type in contentTypes) yield [type.toLowerCase(), type]
...
async function getRelatedTabIds(details) {
  ...
  if (details.tabId != -1) return [details.tabId]
  ...
  if (selector && frameId != -1) {
    try {
      await browser.tabs.sendMessage(
        tabId,
        { type: 'filters.collapse', selector, url },
        { frameId }
      )
    } catch (e) {}
  }
...
function listener(matchFunction, details) {
  ...
  let originUrl = null
  if (details.originUrl) originUrl = parseURL(details.originUrl)
  else if (details.initiator && details.initiator != 'null')
  ...
  if (
    originUrl &&
    (originUrl.protocol == extensionProtocol || originUrl.protocol == 'chrome:')
  )
    return
  ...
  if (!frame && !originUrl) return

  if (checkAllowlisted(page, frame, originUrl)) return

  let type = resourceTypes.get(details.type) || 'OTHER'
  let [docDomain, sitekey, specificOnly] = getDocumentInfo(
    page,
    frame,
    originUrl
  )
...
function urlMatch(
  ...
  if (filter instanceof BlockingFilter) {
    if (typeof filter.rewrite == 'string') {
      ...
      if (redirectUrl != details.url) result = { redirectUrl }
    } else {
      result = { cancel: true }
    }
  }
...
function headerMatch(
  ...
  let match = defaultMatcher.match(
    url,
  ...
  if (!(match instanceof BlockingFilter)) return { filter: match }

  let { blocking } = defaultMatcher.search(
    url,
    contentTypes.HEADER,
    docDomain,
    sitekey,
...
port.on('request.blockedByRTCWrapper', (msg, sender) => {
  ...
  if (checkAllowlisted(page, frame)) return false
...
function onFilterChange(arg, isDisabledAction) {
  ...
  if (ignoreFilterNotifications) return

  // Convert filter text into a filter object.
  if (typeof arg == 'string') arg = Filter.fromText(arg)
  ...
  if (arg instanceof Subscription && arg.filterCount == 0) return

  // Ignore all types of filters but request filters,
  // only these have an effect on the handler behavior.
  if (arg instanceof Filter && !(arg instanceof URLFilter)) return
