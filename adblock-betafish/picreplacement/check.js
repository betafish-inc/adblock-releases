...
const License = (function getLicense() {
  ...
  const mabConfig = {
    ...
    dev: {
      ...
      subscriptionURL:
        'https://dev.getadblock.com/premium/manage-subscription/',
    },
  }
    ...
    browser.alarms.get(License.sevenDayAlarmName, (alarm) => {
      if (alarm && Date.now() > alarm.scheduledTime) {
        ...
        browser.alarms.clear(License.sevenDayAlarmName, (wasCleared) => {
          if (wasCleared) {
            browser.alarms.create(License.sevenDayAlarmName, {
              when: originalTime,
            })
  ...
  return {
    ...
    getCurrentPopupMenuThemeCTA() {
      ...
      currentThemeIndex =
        lastThemeIndex === currentThemeIndex ? 0 : (currentThemeIndex += 1)
      return theme || ''
    },
    ...
    updatePeriodically() {
      ...
      browser.storage.local.get(installTimestampStorageKey).then((response) => {
        ...
        const originalInstallTimestamp =
          installTimestamp || localTimestamp || Date.now()
        ...
        browser.alarms.create(licenseAlarmName, {
          when: nextLicenseCheck.getTime(),
        })
      })
    },
    getLicenseInstallationDate(callback) {
      ...
      browser.storage.local.get(installTimestampStorageKey).then((response) => {
        const localTimestamp = storageGet(installTimestampStorageKey)
        const originalInstallTimestamp =
          response[installTimestampStorageKey] || localTimestamp
    ...
    getFormattedActiveSinceDate() {
      ...
      try {
        ...
        formattedDate = new Date(createdAtUTC).toLocaleDateString(
          undefined,
          dateFormat
        )
      } catch (e) {
    ...
    isLicenseCodeValid() {
      return (
        License && License.get().code && typeof License.get().code === 'string'
      )
    },
    isMyAdBlockEnrolled() {
      return (
        License && License.get() && License.get().myadblock_enrollment === true
      )
    },
    ...
    shouldShowPremiumDcCTA() {
      return (
        License &&
        License.isActiveLicense() &&
        License.get().suppress_premium_cta !== true
      )
    },
    ...
    fetchLicenseAPI(command, requestBody, ok, requestFail) {
      ...
      const request = new Request(
        'https://myadblock.licensing.getadblock.com/license/api/',
        {
          method: 'POST',
...
Promise.all([onInstalledPromise, License.ready(), onBehaviorPromise]).then(
  (detailsArray) => {
...
License.ready().then(() => {
  browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (
      ...
      browser.tabs
        .executeScript(sender.tab.id, {
          file: 'adblock-picreplacement-image-sizes-map.js',
      ...
      browser.tabs
        .executeScript(sender.tab.id, {
          file: 'adblock-picreplacement.js',
          frameId: sender.frameId,
          runAt: 'document_start',
        })
        .catch(logError)
    }
    const subscription = Subscription.fromURL(
    ...
    if (
      ...
      browser.tabs
        .executeScript(sender.tab.id, {
          file: 'adblock-picreplacement-push-notification-wrapper-cs.js',
          runAt: 'document_start',
        })
        .catch(logError)
    }
  ...
  browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (
      request.command === 'showIconBadgeCTA' &&
  ...
  browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (
      request.command === 'License.MAB_CONFIG' &&
