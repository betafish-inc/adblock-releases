...
function topOpenBlacklistUI(options) {
  ...
  class DragElement {
    constructor(el, elementToDrag) {
      ...
      if (document.getElementById(`${el.d}header`)) {
        document.getElementById(`${el.id}header`).onmousedown =
  ...
  loadWizardResources($base, () => {
    ...
    if (options.info && options.info.frameUrl) {
      const frame = $('iframe').filter(
        (i, el) => el.src === options.info.frameUrl
      )
      if (frame.length === 1) {
    ...
    <div id='hiding-wizard'>
      ...
      <div class='page' id='page_1' style='display:none;'>
        ...
        <section>
          ...
          <p class="advanced-user-row" >${translate(
          ...
          <p class="warningText non-advanced-user-text">${translate(
            'sliderwarning'
          )}</p>
      ...
      <div class='page' id='page_3' style='display:none;'>
        ...
        <section>
          <span style="display: flex; flex-direction: column;">
            <span style="font-size: 18px; font-weight: bold;">${translate(
              'wizard_premium_cta_title'
            )}</span>
            <span style="margin-top: 22px;">${translate(
              'wizard_premium_cta_part_I'
            )}  ${translate('wizard_premium_cta_part_II')}</span>
          </span>
          <img aria-hidden='true' src='${browser.runtime.getURL(
            'adblock-wizard_sync_cta.svg'
          )}' style="height: 200px; width: 245px;">
        </section>
        <hr />
        <footer id='blacklist-cta' >
          <button id='find_out_more' class='btnClose'>${translate(
            'find_out_more'
          )}</button>
          <span id='opt-out-msg' class='btnClose'>${translate(
            'dont_show_me_again'
          )}</span>
        </footer>
      </div>
