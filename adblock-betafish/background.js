...
const {
  URLRequest,
} = require('../adblockplusui/adblockpluschrome/adblockpluscore/lib/url.js')
...
const confirmRemovalOfCustomFiltersOnHost = function (host, activeTabId) {
  ...
  const confirmationText = translate('confirm_undo_custom_filters', [
    customFilterCount,
    host,
  ])
  ...
  return new Promise((resolve) => {
    getTab(tabId).then((tab) => {
      ...
      try {
        ...
        const customFilterCheckUrl = info.disabledSite
          ? undefined
          : page.url.hostname
        ...
        const result = {
          ...
          domainPaused: adblockIsDomainPaused({
            url: page.url.href,
            id: page.id,
          }),
          ...
          customFilterCount:
            countCache.getCustomFilterCount(customFilterCheckUrl),
...
const getDebugInfo = function (callback) {
  ...
  for (const id in subscriptions) {
    if (subscriptions[id].subscribed) {
      subscriptionInfo[id] = {}
      subscriptionInfo[id].lastSuccess = new Date(
        subscriptions[id].lastSuccess * 1000
      )
      subscriptionInfo[id].lastDownload = new Date(
        subscriptions[id]._lastDownload * 1000
      )
  ...
  if (localStorage && localStorage.length) {
    ...
    if (edgeMigrationLogs || edgeMigrationLogs.length) {
      response.otherInfo.edgeMigrationLogs = Object.assign(
        {},
        edgeMigrationLogs
      )
    }
      ...
      browser.storage.local.get(excludeFiltersKey).then((secondResponse) => {
        ...
        browser.storage.local.get(errorKey).then((errorResponse) => {
          ...
          browser.storage.local
            .get(migrateLogMessageKey)
            .then((migrateLogMessageResponse) => {
              if (
                migrateLogMessageResponse &&
                migrateLogMessageResponse[migrateLogMessageKey]
              ) {
                const messages =
                  migrateLogMessageResponse[migrateLogMessageKey].split('\n')
              ...
              if (License.isActiveLicense()) {
                ...
                response.otherInfo.licenseInfo.licenseId =
                  License.get().licenseId
                if (getSettings().sync_settings) {
                  response.otherInfo.syncInfo = {}
                  response.otherInfo.syncInfo.SyncCommitVersion =
                    SyncService.getCommitVersion()
                  response.otherInfo.syncInfo.SyncCommitName =
                    SyncService.getCurrentExtensionName()
                  response.otherInfo.syncInfo.SyncCommitLog =
                    SyncService.getSyncLog()
                }
                browser.alarms.getAll((alarms) => {
                  if (alarms && alarms.length > 0) {
                    ...
                    for (let i = 0; i < alarms.length; i++) {
                      ...
                      response.otherInfo[`${i} Alarm Scheduled Time`] =
                  ...
                  License.getLicenseInstallationDate((installdate) => {
                    response.otherInfo['License Installation Date'] =
                      installdate
                    const customChannelId =
                      channels.getIdByName('CustomChannel')
                    if (channels.getGuide()[customChannelId].enabled) {
                      const customChannel =
                        channels.channelGuide[customChannelId].channel
                      customChannel.getTotalBytesInUse().then((result) => {
                        response.otherInfo[
                          'Custom Channel total bytes in use'
                        ] = result
