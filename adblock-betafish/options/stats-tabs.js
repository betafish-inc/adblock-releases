...
const mainTextFontColor = getComputedStyle(document.body)
  ...
  .trim()
const helpIconColor = getComputedStyle(document.body)
  .getPropertyValue('--help-icon-color')
  .trim()
...
const showOrHideAdPanelCountNeeded = function () {
  if (BG.getSettings().local_data_collection && showAdsData) {
    ...
    $('#adsblocked_header span[i18n="stats_hide_data"]').text(
      translate('stats_show_data')
    )
...
const showOrHideTrackerPanelCountNeeded = function () {
  if (
    ...
    if (showTrackerData) {
      ...
      $('#trackers_blocked_header span[i18n="stats_hide_data"]').text(
        translate('stats_show_data')
      )
...
const showOrHideReplacedPanelCountNeeded = function () {
  if (
    ...
    if (showReplacedData) {
      ...
      $('#ads_replaced_header span[i18n="stats_hide_data"]').text(
        translate('stats_show_data')
      )
...
function createDailyLabelData() {
  ...
  for (let i = 0; i < hoursOfThisDay.length; i++) {
    labelData.push(
      hoursOfThisDay[i].toLocaleTimeString(undefined, { hour: '2-digit' })
    )
  }
  return labelData
}
...
function filterTodaysData(chartDataArray) {
  ...
  for (let i = 0; i < chartDataArray.length; i++) {
    for (let j = 0; j < hoursOfThisDay.length; j++) {
      if (
        chartDataArray[i].x >= hoursOfThisDay[j] &&
        chartDataArray[i].x < hoursOfThisDay[j + 1]
      ) {
...
function filterThisWeeksData(chartDataArray) {
  ...
  for (let i = 0; i < chartDataArray.length; i++) {
    for (let j = 0; j < dates.length - 1; j++) {
      if (
        chartDataArray[i].x >= dates[j] &&
        chartDataArray[i].x < dates[j + 1]
      ) {
        dayBuckets[j] += chartDataArray[i].y
      }
    }
    if (
      chartDataArray[i].x >= dates[dates - 1] &&
      chartDataArray[i].x < followingMonday
    ) {
...
function filterThisMonthsData(chartDataArray) {
  ...
  const firstDayOfNextMonth = new Date(
    dates[numberOfDaysThisMonth - 1].getTime()
  )
  ...
  for (let i = 0; i < chartDataArray.length; i++) {
    ...
    if (
      chartDataArray[i].x >= dates[dates - 1] &&
      chartDataArray[i].x < firstDayOfNextMonth
    ) {
...
const sampleChartConfig = {
  ...
  data: {
    ...
    datasets: [
      sampleAdChartDataSet,
      sampleTrackerChartDataSet,
      sampleReplacedChartDataSet,
    ],
  },
  options: {
    events: [],
...
const getDomainTableRow = function (rowNum, filteredDomainData, processedDoms) {
  ...
  return `<div id="domainRow-${rowValue}"class="domainTableRow">
    <div class="siteColumn ellipsis">${rowValue} ${
    processedDoms[rowNum].domain
  }</div>
...
const setFontSize = function (
  ...
  $(totalAdsSelector).css({ 'fontSize': fontsize })
  $(totalTrackersselector).css({ 'fontSize': fontsize })
  $(totalReplacedSelector).css({ 'fontSize': fontsize })
}
          ...
          if (cleanDomain && cleanDomain.length > 1) {
            ...
            if (subs && subs.easyprivacy && subs.easyprivacy.subscribed) {
              domainData[cleanDomain].trackers +=
                rawData[timestamp].doms[domain].trackers
            }
            if (
              License.isActiveLicense() &&
              channels &&
...
const getBarChartConfig = function (chartType, filterName) {
  ...
  for (let inx = 0; inx < processedBarDoms.length && inx < 10; inx++) {
    ...
    barChartTrackersDS.push(
      filteredDomainData[processedBarDoms[inx].domain].trackers
    )
    barChartImageReplsDS.push(
      filteredDomainData[processedBarDoms[inx].domain].adsReplaced
    )
  }
...
const updateChart = function (chartType = 'line', filterName, labelName) {
  ...
  if (chartType === 'line') {
    ...
    ;({ theChartConfig, filteredDomainData, processedDoms } = getBarChartConfig(
      chartType,
      filterName,
      labelName
    ))
...
const initializeStatsTabContent = function () {
  if (!BG.getSettings().local_data_collection) {
    ...
    if (BG.getSettings().data_collection_v2) {
      ...
      $('#btnStatsOptIn').html(
        DOMPurify.sanitize(translate('stats_opt_in_local_text_button'))
      )
      ...
      $('#btnStatsOptIn').html(
        DOMPurify.sanitize(translate('stats_opt_in_text_button'))
      )
    }
    ...
    if (License.isActiveLicense()) {
      if (channels && channels.isAnyEnabled()) {
        ...
        $('#premium_cta_panel .cta_panel_content').text(
          translate('premium_cta_msg_not_enabled')
        )
        $('#btnGetPremium').hide()
...
const resetPageToInitialState = function () {
  ...
  $('#stats-menu-level1 [data-chart-type="line"]')
    .parent()
    .addClass('active-stats-menu-item')
...
$('#stats-menu-level2 a').on('click', function statsOptionLinkClicked() {
  ...
  if (filterFunctionName === 'today') {
    $('#timeBlocks-stats').css({ 'borderTopLeftRadius': '0' })
    $('#siteBlocks-stats').css({ 'borderTopLeftRadius': '0' })
  } else {
    $('#timeBlocks-stats').css({ 'borderTopLeftRadius': '6px' })
    $('#siteBlocks-stats').css({ 'borderTopLeftRadius': '6px' })
  }
...
$('.chart-parent-tab-link').on('click', function statsOptionLinkClicked() {
  ...
  if (chartType === 'line') {
    $('#stats-menu-level2').css({ 'borderTopLeftRadius': '0' })
  } else {
    $('#stats-menu-level2').css({ 'borderTopLeftRadius': '6px' })
  }
...
$(
  '#btnCancelDelete, #delete-overlay-close-icon-page1, #delete-overlay-close-icon-page2'
).on('click', () => {
