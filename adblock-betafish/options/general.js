...
const initialize = function init() {
  ...
  if (
    'acceptable_ads_privacy' in subs &&
    subs.acceptable_ads_privacy.subscribed
  ) {
  ...
  const acceptableAdsPrivacyClicked = function (isEnabled) {
    const acceptableAds = Subscription.fromURL(
      Prefs.subscriptions_exceptionsurl
    )
    const acceptableAdsPrivacy = Subscription.fromURL(
      Prefs.subscriptions_exceptionsurl_privacy
    )
  ...
  $("input.feature[type='checkbox']").on(
    'change',
    function onOptionSelectionChange() {
      ...
      if (name === 'youtube_channel_whitelist') {
        if (isEnabled) {
          ...
          window.setTimeout(() => {
            ...
            $('#youtube_manage_subscribed_link').removeClass('link-text-color')
            $('#youtube_manage_subscribed_link').removeClass('pointer')
            $('#youtube_manage_subscribed_link').addClass(
      ...
      if (name === 'youtube_manage_subscribed') {
        ...
        if (!isEnabled) {
          ...
          $('#youtube_manage_subscribed_link').removeClass(
            'disabled-link-text-color'
          )
        }
      }
...
$('#enable_show_advanced_options').on(
  'change',
  function onAdvancedOptionsChange() {
    ...
    if (!this.checked) {
      $(".advanced input[type='checkbox']:checked").each(
        function forEachAdvancedOption() {
...
$(() => {
  ...
  $('.sync-cta #close-sync-cta-general').on(
    'click',
    MABPayment.userClosedSyncCTA
  )
...
const onSettingsChanged = function (name, currentValue, previousValue) {
  ...
  if (
    checkBoxElement.length === 1 &&
    checkBoxElement.is(':checked') === previousValue
  ) {
