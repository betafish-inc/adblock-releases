...
const onSyncDataInitialGetError = function(errorCode) {
  $("#show-name-div").hide();
  $("#sync-now-div").hide();
  $("#turn-on-div").show();
...
(function(){
  ...
  var deviceNameArray = [];
  const dateFormatOptions = { month: 'long', day: 'numeric', year: 'numeric', hour: 'numeric', minute: "2-digit" };
  ...
  $(document).ready(function () {
    ...
    const getAllExtensionNames = function() {
      SyncService.getAllExtensionNames(function(extensionNameResponse) {
        if (extensionNameResponse && extensionNameResponse.hasData && extensionNameResponse.data) {
          ...
          for (let inx = 0; inx < extensionNameResponse.data.length; inx++) {
            ...
            if (deviceInfo && deviceInfo.deviceName) {
              ...
              $("#sync-extension-list-div").append($("<p></p>").text(deviceInfo.deviceName).addClass("extension-name-item").addClass("bottom-line") );
            }
          }
          const now = new Date();
          const timestampMsg = translate("sync_device_name_list_updated_at_msg", now.toLocaleString(navigator.languages[0], dateFormatOptions));
    ...
    const onLicenseUpdating = function() {
      ...
      $(".sync-header-message").removeClass("sync-header-message-hidden").addClass("sync-header-message-normal").removeClass("sync-message-error");
    };

    const onLicenseUpdated = function(licenseObj) {
      removeSyncListeners();
      location.reload();
    };

    const onLicenseUpdatedError = function() {
      // Currently commented out - get error handling beyond the above code is not defined for this version
    };
    ...
    const onSettingsChanged = function(name, currentValue, previousValue) {
      ...
      // if another options page is opened, and a change is made to the sync setting, reload this page
    ...
    $("#btnTurnSyncOn").click((event) => {
      $("#turn-on-div").fadeOut('slow', function() {
    ...
    $("#btnVerifyCancel").click((event) => {
      $("#verify-overwrite-div").fadeOut('slow', function() {
        $("#btnTurnSyncOn").addClass("red");
        $("#turn-on-div").fadeIn('slow');
      });
    });

    $("#btnVerifyOK").click((event) => {
      $("#verify-overwrite-div").fadeOut('slow', function() {
        $("#error-message").text("");
        $("#enter-name-div").show();
        $("#enter-name-sub-div").css('display', 'flex');
    ...
    $("#btnTurnSyncOff").click((event) => {
      ...
      $("#show-name-div").fadeOut('slow', function() {
        $("#error-message").text("");
        $("#btnTurnSyncOn").addClass("red");
        $("#turn-on-div").fadeIn('slow');
        $("#extension-name").val("");
    ...
    $("#btnSaveSyncName").click((event) => {
      ...
      $("#enter-name-div").fadeOut('slow', function() {
    ...
    if (License.isActiveLicense() && License.get() && !License.get().licenseId) {
      ...
      $(".sync-tab-message").removeClass("sync-tab-message-hidden").addClass("sync-tab-message-normal");
      $("#no-license-div").show();
        ...
        if (currentExtensionName && backgroundPage.getSettings().sync_settings) {
          processReplacementChildren($("#show-name-message"), currentExtensionName, "sync_show_name_text");
          $("#show-name-div").show();
          $("#sync-now-div").show();
          return;
        }
