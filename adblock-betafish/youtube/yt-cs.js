...
if (
  ...
  /^www\.youtube\.com$/.test(window.location.hostname) === true
) {
  /*
  ...
  */
  ...
  const toContentScriptRandomEventName = `ab-yt-channel-name-${Math.random()
    .toString(36)
    .substr(2)}`
  const fromContentScriptRandomEventName = `yt-ab-channel-name-${Math.random()
    .toString(36)
    .substr(2)}`
  ...
  const onMessage = function (request, sender, sendResponse) {
    if (request.command === 'updateURLWithYouTubeChannelName') {
      ...
      const thisChannelList = document.querySelectorAll(
      ...
      if (thisChannelList.length > 0 && thisChannelList[0].innerText) {
        ...
        if (
          tempChannelName &&
          gNextVideoId === parseUri.parseSearch(window.location.href).v
        ) {
          window.postMessage(
            {
              eventName: fromContentScriptRandomEventName,
  ...
  const captureAJAXRequests = function (
