  ...
  <div id="onPageIcon">
    <div id="messagePanel" >
      <div id="message-bubble-content" >
        <div id="lineOne" >
          <img id="lineOneIconLogo" type="image/svg+xml" src='${browser.runtime.getURL(
            'adblock-ads-blocked-icon.svg'
          )}' ></img>
          <span id="lineOneText" ></span>
        </div>
        <div id="lineTwo" >
          <img id="lineTwoIconLogo" type="image/svg+xml" src='${browser.runtime.getURL(
...
const getAllSubscribedChannelNames = function () {
  ...
  for (let inx = 0; inx < channelNodes.length; inx++) {
    ...
    const channelNameNode = channelNode.querySelectorAll(
...
const buildOverlay = function () {
  ...
  const blockedCompleteText = translate(
    'youtube_onpage_icon_blocked_complete_text'
  )
  const allowedCompleteText = translate(
    'youtube_onpage_icon_allowed_complete_text'
  )
  ...
  const noChannelsFoundMessageText = translate(
    'youtube_onpage_icon_no_channels_text'
  )
  ...
  const normalContentString = `
      ...
      <div id="buttonContent">
        <div id="btnAllowAll" class="onPageIconButton" >
          <div class="centerContent" >
            <img class="smallIconLogo" type="image/svg+xml" src='${browser.runtime.getURL(
              'adblock-ads-allowed-icon.svg'
            )}' ></img>
        ...
        <div id="btnBlockAll" class="onPageIconButton">
          <div class="centerContent" >
            <img class="smallIconLogo" src='${browser.runtime.getURL(
              'adblock-ads-blocked-icon.svg'
            )}' ></img>
    ...
    <div id="allowAllDoneContent" >
        <img class="largeIconLogo" src='${browser.runtime.getURL(
          'adblock-ads-allowed-icon.svg'
        )}' ></img>
  ...
  if (document.querySelectorAll('#guide-renderer').length === 0) {
    ...
    overlayChildString =
      overlayPrefix + noChannelsFoundMessageContent + overlaySuffix
  } else {
...
const addOnPageIcon = function () {
  ...
  if (mainBody) {
    ...
    const signInMessageContent = overlayChildElement.querySelector(
    ...
    const processingContent =
      overlayChildElement.querySelector('#processingContent')
    const allowAllDone = overlayChildElement.querySelector(
      '#allowAllDoneContent'
    )
    const blockAllDone = overlayChildElement.querySelector(
      '#blockAllDoneContent'
    )
    const hoverOverIconElem =
      overlayChildElement.querySelector('.hoverOverIcon')
    ...
    if (btnAllowAll) {
      btnAllowAll.onclick = function closedClicked(event) {
        ...
        browser.runtime
          .sendMessage({
            command: 'allowAllSubscribedChannel',
            channelNames: channelNameObs,
          })
          .then(() => {
    ...
    if (btnBlockAll) {
      btnBlockAll.onclick = function closedClicked(event) {
        ...
        browser.runtime
          .sendMessage({
            command: 'blockAllSubscribedChannel',
            channelNames: channelNameObs,
          })
          .then(() => {
            // eslint-disable-next-line no-use-before-define
...
const configureChildElements = function (parentNodeArg, adsAllowedArg) {
  ...
  const lineOneClickHandler = function (event) {
    ...
    browser.runtime
      ...
      .then(() => {
        browser.runtime
          .sendMessage({ command: 'getAllAdsAllowedUserFilters' })
          .then((response) => {
            const ytRules = response.adsAllowedUserFilters
            const { parsedChannelName } = parentNode.dataset
            parentNode.dataset.filterText = isAdsAllowed(
              ytRules,
              parsedChannelName
            )
  ...
  if (adsAllowed) {
    ...
    lineTwoTextElem.textContent = translate(
      'youtube_onpage_icon_allowing_ads_on_channel'
    )
  } else {
    ...
    lineOneTextElem.textContent = translate(
      'youtube_onpage_icon_blocking_ads_on_channel'
    )
  ...
  browser.runtime
    ...
    .then((userFilters) => {
      ...
      for (const [channelName] of Object.entries(channelNameObs)) {
        ...
        channelNameObs[channelName].isAdsAllowed = isAdsAllowed(
          youTubeFilters,
          name
        )
      ...
      if (initialAdd) {
        for (const [channelName] of Object.entries(channelNameObs)) {
          ...
          const onPageBaseShadow = onPageClickIconParentElem.attachShadow({
            mode: 'open',
          })
          ...
          onPageClickIconStyleElement.textContent =
            onPageClickIconStyleTextContent
          ...
          theNode.dataset.parsedChannelName =
            channelNameObs[channelName].parsedChannelName
          theNode.dataset.channelName = channelName

          configureChildElements(
            theNode,
            channelNameObs[channelName].isAdsAllowed
          )

          const notifcationElem = channelNameObs[
            channelName
          ].node.querySelector('#notification-preference-button')
          if (notifcationElem) {
...
const toContentScriptRandomEventName = `ab-yt-event-${Math.random()
  .toString(36)
  .substr(2)}`
